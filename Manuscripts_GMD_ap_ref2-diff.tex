...
%DIF DEL Manuscripts_GMD_ap.tex        Thu Sep 21 15:04:17 2017
%DIF ADD Manuscripts_GMD_ap_ref1.tex   Thu Sep 21 23:21:13 2017
...
cross with experimental results of JPUE (jet or plume that is ejected from a nozzle into a uniform environment)\DIFdelbegin \DIFdel{and the top height of the }\DIFdelend \DIFaddbegin \DIFadd{. 
%DIF > The model is validated by comparing against simulation results from other 3D plume models.
Profiles of several integrated variables are compared with those calculated by existing 3D plume models for an eruption with the same MER (mass eruption rate) estimated for the }\DIFaddend Pinatubo eruption of \DIFdelbegin \DIFdel{15 June }\DIFdelend \DIFaddbegin \DIFadd{June 15 }\DIFaddend 1991. Our results are consistent with \DIFdelbegin \DIFdel{both observations and }\DIFdelend existing 3D plume models. \DIFdelbegin \DIFdel{Profiles of several integrated variables are compared with those calculated in existing 3D plume models, and further verify our model. }\DIFdelend Analysis of the plume evolution process \DIFdelbegin \DIFdel{illustrates }\DIFdelend \DIFaddbegin \DIFadd{demonstrates }\DIFaddend that this model is able to reproduce the physics of plume development.
...
Even though SPH has been known for several decades, implementations of SPH for compressible multiphase turbulent flows are few. \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{colagrossi2003numerical}
}%DIFAUXCMD
proposed a multiphase SPH model for numerical simulation of air entrapment in violent fluid-structure interactions. \mbox{%DIFAUXCMD
...
also proposed new formulations for weakly compressible fluid with speed of sound sufficiently large to guarantee that the relative density variations are typically $1\%$. \mbox{\citet{chen2015sph} }} \DIFadd{recently presented a new SPH model for weakly compressible multiphase flows with complex interfaces and large density differences. All of them focus on incompressible or weakly compressible flow.
}
...
\item Corrected formulation of SPH \citep{chen1999improvement} is \DIFdelbegin \DIFdel{adapted }\DIFdelend \DIFaddbegin \DIFadd{adopted }\DIFaddend to bypass the well-known tensile instability issues of classical SPH.
\item Simulation of volcanic plumes with acceptable accuracy requires fine resolution (very high particle counts) that cannot be accomplished without parallel computing \DIFaddbegin \DIFadd{using large process counts}\DIFaddend . The core solver of our model is parallelized by \DIFaddbegin \DIFadd{distributed memory }\DIFaddend MPI (message passing interface standard) parallelism. In addition, a dynamic load balancing strategy is also developed. 
...
\end{equation}
\DIFdelbegin \DIFdel{Where}\DIFdelend \DIFaddbegin \DIFadd{where}\DIFaddend , $C_v$ is the specific heat with constant volume, $R$ is the gas constant. $\xi$ \DIFdelbegin \DIFdel{with subscript }\DIFdelend is the mass fraction of\DIFdelbegin \DIFdel{corresponding constituent}\DIFdelend \DIFaddbegin \DIFadd{erupted material}\DIFaddend. The subscript $m$ represents mixture of ejected material and air, $s$ represents solid portion in the ejected material, $g$ represents gas portion in the ejected material\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $a$ represents air\DIFaddbegin \DIFadd{, $0$ represents phyiscal properties of erupted material}\DIFaddend . $\xi_{g0}$ is the mass fraction of vapor in the erupted material
...
In the current model the initial domain is a box. The boundaries are categorized as the velocity inlet (a circular area at the center of the bottom of the box), wall boundary (box bottom) \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and } \DIFaddend pressure outlet (other faces of the box).
...
\dfrac{\partial e}{\partial n}=\dot M e\DIFaddbegin \DIFadd{_0 }\DIFaddend /\left(\pi r^2\right) \label{eq:erupt_bc_e}
\end{align} 
\DIFdelbegin \DIFdel{Where }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend $r$ is the radius of the vent, $n$ is the direction normal to the boundary\DIFaddbegin \DIFadd{, $e_0$ is specific internal energy of erupted material which can be calculated based on temperature and specific heat of erupted material}\DIFaddend .
...
\end{equation}
The Dirac function \DIFaddbegin \DIFadd{$\delta$ }\DIFaddend in Eq. (\ref{eq:Dirac-translation}) can be approximated by a weighting function $w\left(\textbf{x}-\textbf{x}\prime, h\right)$ (or $w\left(\textbf{x}\prime-\textbf{x}, h\right)$) \DIFdelbegin \DIFdel{) }\DIFdelend which tends to a Dirac function when the smoothing length $h \rightarrow 0$ :
\begin{equation}
...
\end{equation}
\DIFdelbegin \DIFdel{An }\DIFdelend \DIFaddbegin \DIFadd{The weighting function, as an approximate form of the Dirac function, should satisfy the normalization condition:
...
\subsection{\DIFdelbegin \DIFdel{Weighting function}\DIFdelend \DIFaddbegin \DIFadd{Artificial Viscosity}\DIFaddend } \DIFdelbegin \DIFdel{As described in the previous section, the weighting function (or kernel function) is used to replace the Dirac function and should satisfy Eq. (\ref{eq:SPH_kernel_delta}). So it can be viewed as an approximate form of the Dirac function and satisfies the normalization condition:
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \label{sec:artificial-viscosity}
...
\DIFdelbegin \DIFdel{There is a wide variety of possible weighting functions, such as spline functions (with different orders) and Gaussian functions. Generally, the accuracy increases with the order of the polynomials of the kernel function, but the computational cost also increases as number of interactions increase. We are adopting a truncated Gaussian function as the weighting function }\DIFdelend \DIFaddbegin \DIFadd{One of the most commonly used models of artificial viscosity \mbox{%DIFAUXCMD
...
\DIFadd{\bar{\rho}_{ab} = \dfrac{\rho_a + \rho_b}{2} }\\
\DIFadd{\textbf{v}_{ab}=\textbf{v}_a-\textbf{v}_b }\\
\DIFadd{\textbf{x}_{ab}=\textbf{x}_a-\textbf{x}_b
}\end{align}
...
. It has been shown that  shear viscosity coefficient $\eta = \frac{\rho \alpha h c}{8} $ and bulk viscosity coefficient $ \zeta = \frac{5 \eta}{3}$ are appropriate for two dimensional flows. $\eta = \frac{\rho \alpha h c}{10} $ and $ \zeta = \frac{5 \eta}{3}$ are appropriate for three dimensional flows.
...
$\alpha = 1$ and $\beta = 2$ are  recommended by Monaghan for best results. In our simulation, these two parameters are calibrated to  $\alpha = 0.3$ and $\beta = 0.6$}\DIFaddend . \DIFdelbegin \begin{displaymath}\DIFdel{
...
\end{equation}
\DIFdelbegin \DIFdel{Where }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend $c_a$ is sound speed at particle \DIFdelbegin \DIFdel{a, $d$ is number of dimensions. }\DIFdelend \DIFaddbegin \DIFadd{$a$ and calculated based on heat specific ration of the mixture $\gamma_m$ (See Eq. (\ref{eq:sound-speed-mixture})). 
}\begin{equation}\DIFadd{
c_a = \left( \gamma_m \frac{p}{\rho} \right)^{0.5}
\label{eq:sound-speed-mixture}
}\end{equation}

\DIFaddend First order Euler integration, with $\textrm{CFL} = 0.2$, is used to advance in time.
...
. }\DIFaddend To address these difficulties, \DIFdelbegin \DIFdel{Chen \mbox{%DIFAUXCMD
\citep{chen1999improvement}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{chen1999improvement}
}%DIFAUXCMD
}\DIFaddend proposed a corrected SPH formulation. For 1D case, employing a Taylor expansion for $A\left(x\right)$ about $x_a$, multiplying both sides by kernel function and then doing integration over the domain gives
\begin{equation}
...
\begin{equation}
A_a = \DIFdelbegin \DIFdel{\frac{\sum_b m_b \dfrac{A_b}{\rho_b} w\left(x_a-x_b, h\right)}{\sum_b m_b \dfrac{1}{\rho_b} w\left(x_a-x_b, h\right)}
}\DIFdelend \DIFaddbegin \DIFadd{\frac{\sum_b m_b \dfrac{A_b}{\rho_b} w_{ab}\left(h\right)}{\sum_b m_b \dfrac{1}{\rho_b} w_{ab}\left(h\right)}
}\DIFaddend \label{eq:CSP-function-approximation-1d}
\end{equation}
\DIFdelbegin \DIFdel{Equation }\DIFdelend \DIFaddbegin \DIFadd{Notice that the denominator in Eq. }\DIFaddend (\ref{eq:CSP-function-approximation-1d}) \DIFdelbegin \DIFdel{implies that $\sum_b m_b \dfrac{1}{\rho_b} w\left(x-x_b, h\right) = 1$, which can be viewed as the approximation form of }\DIFdelend \DIFaddbegin \DIFadd{is actually summation approximation of the left side in }\DIFaddend Eq. (\ref{eq:SPH-kernel-normalization-prop}). \DIFaddbegin \DIFadd{That is to say, Eq. (\ref{eq:CSP-function-approximation-1d}) and Eq. (\ref{eq:SPH-approximation-sum}) are the same for particles far away from boundaries as the denominator in Eq. (\ref{eq:CSP-function-approximation-1d}) becomes $1$ in that case.
...
\end{equation}
\begin{equation}
<\rho_\alpha^{sg}>=\DIFdelbegin \DIFdel{\frac{\sum_j m_j w_{\alpha j} \left(h\right)}{\sum \frac{m_b}{\rho_b} w_{\alpha b} \left(h\right) +\sum \frac{m_j}{\rho_j} w_{\alpha j} \left(h\right)} }\DIFdelend \DIFaddbegin \DIFadd{\frac{\sum m_j w_{\alpha j} \left(h\right)}{\sum \frac{m_b}{\rho_b} w_{\alpha b} \left(h\right) +\sum \frac{m_j}{\rho_j} w_{\alpha j} \left(h\right)} }\DIFaddend \label{eq:gov-sph-d2}
\end{equation}
\DIFdelbegin \DIFdel{Where }\DIFdelend \DIFaddbegin \DIFadd{where }\DIFaddend the subscript $a$ and $b$ represents air particles (phase 1) while $i$ and $j$ represents \DIFaddbegin \DIFadd{particles of }\DIFaddend erupted material. $\beta$ and $\alpha$ represents either erupted material \DIFdelbegin \DIFdel{or air}\DIFdelend \DIFaddbegin \DIFadd{particles or air particles}\DIFaddend .
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=12cm]{Interface.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=12cm]{Fig1}
...
\DIFdelbegin \DIFdel{Any implementation of SPH code requires efficient searching and updating of neighbors during simulation}\DIFdelend \DIFaddbegin \DIFadd{TThe time complexity for an SPH method is $O(N^2)$, where, $N$ is the total number of SPH particles. Efficient neighbor searches and compact-supported kernel functions can help to reduce computational cost.}\DIFaddend . Of the many possible choices we adopt a background grid which was proposed by \citet {monaghan1985refined} and is quite popular in parallel SPH. \DIFaddbegin \DIFadd{Then the time complexity is reduced to $O(MN+mN)$. Where $m$ is the average of number of particles within the compact support of kernel function, $M$ is average number of particles in sub-domain within which neighbour searching is carried out. }\DIFaddend This background grid is also used for domain decomposition\DIFdelbegin \DIFdel{in SPH}\DIFdelend . We refer to the elements of background grid, namely squares for two dimensions and cubic for three dimensions, as buckets. 
...
\DIFdelbegin \DIFdel{As for domain decomposition, even }\DIFdelend \DIFaddbegin \DIFadd{The parallelization is achieved by splitting computational domain into subdomains. Each subdomain is computed by a single processor. For any subdomain, information from its neighbouring subdomains is required when updating physical quantities. To guarantee consistency, data is synchronized if physical quantities are updated. Even }\DIFaddend though more complicated graph-based partitioning tools \citep {biswas1999experiments} might get higher quality decomposition, they requires much more effort in programming and computation. So we adopt an easy-programming scheme based on SFC \citep {patra1999efficient} \DIFaddbegin \DIFadd{to decompose the computational domain}\DIFaddend . Subfigure $c$ of Fig. \ref{fig:bc_and_domain_decomp} shows a cross section view of domain decomposition.

More details about the data structure, domain decomposition, load balancing\DIFdelbegin \DIFdel{and domain adjusting will be published separately }\DIFdelend \DIFaddbegin \DIFadd{, domain adjusting and performance benchmarking has been published separately \mbox{%DIFAUXCMD
\citep{cao2017data}
}%DIFAUXCMD
}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=0.45]{Fig4}
...
%DIFDELCMD < \includegraphics[width=8cm]{vel_cross}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[!htb]
    \centering
	  \includegraphics[width=16cm]{Fig7}
    \DIFaddendFL \caption{Dimensionless \DIFaddbeginFL \DIFaddFL{concentration and }\DIFaddendFL velocity distribution across the cross-section.}
    \DIFdelbeginFL %DIFDELCMD < \label{fig:JPUE_cross-section_vel}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:JPUE_cross-section}
\DIFaddendFL \end{figure}
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \includegraphics[width=8cm]{conc_cross}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Dimensionless concentration distribution across the cross-section.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:JPUE_cross-section_conc}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend 

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \includegraphics[width=8cm]{velo_along_axis}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[!htb]
    \centering
	\includegraphics[width=16cm]{Fig8} 
    \DIFaddendFL \caption{\DIFdelbeginFL \DIFdelFL{Dimensionless }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The left plot is normalized jet width (which determined based on }\DIFaddendFL velocity\DIFdelbeginFL \DIFdelFL{distribution }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{) }\DIFaddendFL along \DIFdelbeginFL \DIFdelFL{centraline}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the centerline}\DIFaddendFL . \DIFaddbeginFL \DIFaddFL{The right plot shows normalized concentration along the centerline.}\DIFaddendFL }
    \DIFdelbeginFL %DIFDELCMD < \label{fig:JPUE_along-axis_vel}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:JPUE_along-axis}
\DIFaddendFL \end{figure}
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \includegraphics[width=8cm]{conc_along_axis}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdel{Dimensionless concetration distribution along centraline.}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:JPUE_along-axis_conc}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend
...
}\DIFdelend \DIFaddbegin \DIFadd{(See Eq. (\ref{eq:JPUE-experiment-fit-corss})) by \mbox{%DIFAUXCMD
\citet{papanicolaou1988investigations}
}%DIFAUXCMD
and  \mbox{%DIFAUXCMD
\citet{ george1977turbulence}
}%DIFAUXCMD
}\DIFaddend even though the actual profile are slightly different from the Gaussian profile.
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{papanicolaou1988investigations}
}%DIFAUXCMD
also fit concentration distribution and jet width based on velocity along central line into a straight line : $\dfrac{\varphi_0}{\varphi_c}=slope \left(\dfrac{z}{D} + intercept \right)$ (dash line). }\DIFdelend \DIFaddbegin 
...
also fit concentration distribution and jet width based on velocity along centerline into a straight line (See Eq. (\ref{eq:JPUE-experiment-fit-along-axis})).. 
}
...
Vertical distribution of temperature, pressure and density is used to establish stratified atmosphere. \DIFaddbegin \DIFadd{Wind velocity and specific humidity are not used in our simulation even though they were also provided by \mbox{%DIFAUXCMD
\citet{costa2016results}
}%DIFAUXCMD
(see Fig. 1B). }\DIFaddend Material properties, shown in Table \ref{tab:material_properties}, are selected based on properties of Pinatubo and Shinmoe-dake eruption. Other material properties not given in the table can be \DIFdelbegin \DIFdel{infered }\DIFdelend \DIFaddbegin \DIFadd{inferred }\DIFaddend from these given parameters based on their relationships.
...
\begin{figure}
\DIFaddendFL \includegraphics[width=15cm]{Fig12}
...
\end{figure}
\DIFaddend Our model adopts the same assumptions and governing equations as SK-3D. However, there is still \DIFdelbegin \DIFdel{a big }\DIFdelend \DIFaddbegin \DIFadd{an obvious }\DIFaddend disparity between the profiles of local variables of our model and SK-3D. One of the big differences between these two models is that we adopt a LANS type of turbulence model while SK-3D adopts a LES (large eddy simulation) turbulence model. This implies that \DIFaddbegin \DIFadd{choice of  }\DIFaddend turbulence model might play a critical role in plume simulation.
...
A new plume model was developed based on the SPH method. Extensions necessary for Lagrangian methodology and compressible flow were made in the formulation of the equations of motion and turbulence models. Advanced numerical techniques in SPH were exploited and tailored for this model. High performance computing was used to mitigate the tradeoff between accuracy (depends on comprehensiveness of the model, resolution, order of accuracy of numerical methods, scheme for time upgrading\DIFdelbegin \DIFdel{.}\DIFdelend ) and simulation time (depends on comprehensiveness of model, resolution, order of accuracy of numerical methods, scheme for time upgrading ... and computational techniques). The correctness of the code and model was verified and validated by a series of test simulations. \DIFaddbegin \DIFadd{Typical 1D shock tube problems were simulated and compared against analytical results showing good agreement. }\DIFaddend Dimensionless velocity and concentration distribution across the cross-section and along the jet axis match well with experimental results of JPUE. Top height and integrated local variables simulated by our model are consistent with simulation results of existing 3D plume models. Comparison between our results with \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{those }\DIFaddend of SK-3D implies that turbulence model plays a significant role in plume modeling.
...
\appendix
\section{\DIFadd{Post processing of particle data}} \label{app:project-SPH-grid}    %DIF > % Appendix A
\appendixfigures
\begin{figure}[!htb]
    \centering
...
\DIFadd{Particles distribute irregularly in SPH simulation results. To adapt post processing originally proposed for mesh-based method, we need to project simulation results onto a pre-defined regular mesh. As shown in Fig. \ref{appfig:1D-shock-tests-verification}, the basic steps for such projection are:
}\begin{itemize}
\item \DIFadd{obtain raw simulation results carried by particles that irregularly distribute in the space
}\item \DIFadd{create regular grids
}\item \DIFadd{search for neighbour paricles for each node of the regular grids.
}\item \DIFadd{interpolate physical quanties from neighbour particle onto corresponding node of regular grids according to Eq. (\ref{eq:SPH-fundamental-principle})
}\end{itemize}
...
\DIFdelbegin %DIFDELCMD < \bibliography{Reference.bib}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \bibliography{Reference}
\DIFaddend %% Since the Copernicus LaTeX package includes the BibTeX style file copernicus.bst,
